...
public class DbIndex {
	...
	public synchronized String findPhoneData(String _number){
		...
		try{
			...
			if(_number.length() < 11){
				...
				return YuchCaller.sm_local.getString(yuchcallerlocalResource.PHONE_LOCAL_PHONE);
		...
		return  YuchCaller.sm_local.getString(yuchcallerlocalResource.PHONE_UNKNOWN_NUMBER);
	}
	
	//! search the phone data
	private PhoneData searchPhoneData(String _cityNumber){
		
		int t_num4 = Integer.parseInt(_cityNumber);
	...
	private CellPhoneData searchCellPhoneData(String _cellPhone){
		int t_num7 = Integer.parseInt(_cellPhone.substring(0,8));
